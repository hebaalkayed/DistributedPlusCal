     ...
     **********************************************************************/
     { try {
       ...
       if (PeekAtAlgToken(1).equals("variable") || PeekAtAlgToken(1).equals("variables")) {
    	   vdecls = GetVarDecls();
    	   PcalDebug.reportInfo("variable declarations found and successfully added.");
       }
       ...
       if (PeekAtAlgToken(1).equals("channel") || PeekAtAlgToken(1).equals("fifo")
    	   ...
    	   PcalDebug.reportInfo("channel/fifo declarations found and  successfully added.");
       }
       ...
       TLAExpr defs = new TLAExpr();
		...
		if(PcalParams.distpcalFlag) {
			...
			if (PeekAtAlgToken(1).equals("node") || PeekAtAlgToken(1).equals("process")
				...
				while (i < multiNodes.nodes.size()) {
					...
					while (j < node.threads.size()) {
						// the execution enters this while loop always
				...
				while (i < multiNodes.prcds.size()) {
					// if there are no procedures execution does not enter this block of while loop
				...
				return multiNodes;
			} // multi-process part end DISTRIBUTED PLUSCAL
			
			else {
				// uni process part begin DISTRIBUTED PLUSCAL
				PcalDebug.reportInfo("Distributed Pluscal Uni process else statement");
			...
			// edit for expanding channel expression in uni-process specs
			...
			// FOR MULTI-PROCESS ALGORITHMS PLUSCAL
			if ((PeekAtAlgToken(1).equals("fair") 
...
//      }
   ...
   public static AST.Node GetNode() throws ParseAlgorithmException {
		...
		if (PeekAtAlgToken(1).equals("begin") || PeekAtAlgToken(1).equals("{") || PeekAtAlgToken(1).equals("subprocess")) {
			...
			// enters only if we declared local variables/channels inside process
		...
		// edit. build main sub-process first and then look for sub-processes in while loop.
		...
		*/
		...
		if (PeekAtAlgToken(1).equals("{") || PeekAtAlgToken(1).equals("subprocess")) {
			PcalDebug.reportInfo("Varible declarations inside subprocess");
